...
function defaultDB() {
  return {
    ...
    last_mana: null, buffs_cast: 0, bosses_slain: 0, all_time_damage: 0,
    weekly_damage: 0, peak_daily_damage: 0, current_day_damage: 0,
    damage_day_date: '', last_damage_up: 0, weekly_top_dailies: {},
    last_daily_date: '', daily_habit_baseline: 0,
    last_quest_key: null, last_quest_is_boss: false,
...
function mergeClickDicts(...dicts) {
  ...
  for (const d of dicts) {
    for (const [tid, entry] of Object.entries(d)) {
      const m = merged[tid] || { text: entry.text, count: 0 };
      m.text = entry.text;
      m.count += entry.count;
      merged[tid] = m;
    }
...
async function generateSVG() {
  ...
  const mp = (uData.stats && uData.stats.mp) || 0;

  if (!db.classes_used.includes(cClass)) db.classes_used.push(cClass);
  db.peak_gold = Math.max(db.peak_gold, gold);
  ...
  const dailies = tRes.filter((t) => t.type === 'daily');
  const due = dailies.filter((t) => t.isDue);
  const done = due.filter((t) => t.completed);
  const pct = due.length ? Math.round((done.length / due.length) * 100) : 100;
  const dailiesGagal = Math.max(0, due.length - done.length);

  const currentCompletedIds = new Set(done.map((d) => d.id));
  const lastCompletedIds = new Set(db.last_completed_daily_ids || []);
  ...
  for (const dTask of done) {
    if (newlyCompleted.includes(dTask.id)) {
      ...
      bump(db.weekly_top_dailies, dTask.id, text, 1);
    }
  }
  ...
  const up = habits.reduce((s, h) => s + (h.counterUp || 0), 0);
  const dn = habits.reduce((s, h) => s + (h.counterDown || 0), 0);
  const hratio = up + dn > 0 ? Math.round((up / (up + dn)) * 100) : 100;

  if (db.all_time_habits_pos === 0 && up > 0) db.all_time_habits_pos = up;
  if (db.all_time_habits_neg === 0 && dn > 0) db.all_time_habits_neg = dn;
  ...
  for (const h of habits) {
    const hid = h.id;
    const text = (h.text || '').slice(0, 28);
    const curUp = h.counterUp || 0;
    const curDn = h.counterDown || 0;
    const prev = lastHabitCounters[hid] || { up: curUp, down: curDn };
    const deltaUp = Math.max(0, curUp - (prev.up ?? curUp));
    const deltaDn = Math.max(0, curDn - (prev.down ?? curDn));

    if (deltaUp > 0) {
      db.all_time_habits_pos += deltaUp;
      bump(weeklyHabitClicks, hid, text, deltaUp);
      bump(todayHabitClicks, hid, text, deltaUp);
      bump(monthlyHabitClicks, hid, text, deltaUp);
    }
    if (deltaDn > 0) {
      db.all_time_habits_neg += deltaDn;
      bump(weeklyHabitNeg, hid, text, deltaDn);
      bump(todayHabitNeg, hid, text, deltaDn);
    }
    lastHabitCounters[hid] = { up: curUp, down: curDn };
  }

  db.last_habit_counters = lastHabitCounters;
  db.weekly_habit_clicks = weeklyHabitClicks;
  db.weekly_habit_neg = weeklyHabitNeg;
  db.today_habit_clicks = todayHabitClicks;
  db.today_habit_neg = todayHabitNeg;
  ...
  const topH = Object.values(weeklyHabitClicks).sort((a, b) => b.count - a.count).slice(0, 5);
  const topHNeg3 = Object.values(weeklyHabitNeg).sort((a, b) => b.count - a.count).slice(0, 3);
  const topHMonth = Object.values(monthlyHabitClicks).sort((a, b) => b.count - a.count).slice(0, 5);

  const touchedIdsWeek = new Set([...Object.keys(weeklyHabitClicks), ...Object.keys(weeklyHabitNeg)]);
  const habitsUntouchedWeek = habits.filter((h) => !touchedIdsWeek.has(h.id)).length;
  ...
  const hToday = Math.max(0, db.all_time_habits_pos - db.daily_habit_baseline);
  const tToday = cRes.filter((t) => {
    if (!t.dateCompleted) return false;
    const dWIB = new Date(new Date(t.dateCompleted).getTime() + WIB_MS);
    return fmtDateStr(dWIB) === todayStr;
  }).length;
...
`;
  ...
  let pineTrees = '';
  for (let i = 0; i < 16; i++) {
    const x = randInt(rnd, -30, 430);
    const scale = randFloat(rnd, 0.7, 1.3);
  ...
  let nightStars = '';
  for (let i = 0; i < 30; i++) {
    const sx = randInt(rnd, 5, 455);
    const sy = randInt(rnd, 5, 125);
    const r = randFloat(rnd, 0.6, 1.8);
    const op = randFloat(rnd, 0.4, 0.95);
    nightStars += `<circle cx="${sx}" cy="${sy}" r="${r.toFixed(2)}" fill="#fef9e7" opacity="${op.toFixed(2)}"/>`;
  }

  const groundBand = '<rect x="0" y="90" width="460" height="50" fill="url(#groundGrad)"/>';
  rnd = seededRandom(33);
  let grass = '';
  for (let i = 0; i < 45; i++) {
    const gx = randInt(rnd, 0, 460);
    const gh = randFloat(rnd, 5, 12);
    const gy = 140 - gh;
    const op = randFloat(rnd, 0.5, 0.9);
    grass += `<polygon points="${gx - 2},140 ${gx},${gy.toFixed(1)} ${gx + 2},140" fill="#15803d" opacity="${op.toFixed(2)}"/>`;
  }
  ...
  const icTg = '<circle cx="12" cy="12" r="8" stroke="#0284c7" stroke-width="2" fill="none"/><circle cx="12" cy="12" r="3" fill="#0284c7"/>';
  const icSt = '<path d="M12 2L15 9L22 9L16 14L18 21L12 17L6 21L8 14L2 9L9 9Z" fill="#ca8a04"/>';
  const icSparkle = '<path d="M12 2L14 9L21 11L14 13L12 20L10 13L3 11L10 9Z" fill="#facc15"/>';
  const icDrop = '<path d="M12 2C12 2 5 11 5 15.5C5 19.6 8.1 22 12 22C15.9 22 19 19.6 19 15.5C19 11 12 2 12 2Z" fill="#60a5fa"/>';
  const icThumbUp = '<path d="M2 21H6V10H2V21ZM9 21H17C17.8 21 18.5 20.4 18.7 19.6L21 12.4C21.3 11.4 20.5 10.5 19.5 10.5H14.4L15 6.5C15.1 5.7 14.5 5 13.7 5C13.3 5 12.9 5.2 12.6 5.5L8.3 10.3C8.1 10.5 8 10.8 8 11.1V19.5C8 20.3 8.7 21 9 21Z" fill="#22c55e"/>';
  const icThumbDown = '<path d="M22 3H18V14H22V3ZM15 3H7C6.2 3 5.5 3.6 5.3 4.4L3 11.6C2.7 12.6 3.5 13.5 4.5 13.5H9.6L9 17.5C8.9 18.3 9.5 19 10.3 19C10.7 19 11.1 18.8 11.4 18.5L15.7 13.7C15.9 13.5 16 13.2 16 12.9V4.5C16 3.7 15.3 3 15 3Z" fill="#ef4444"/>';
  const icX = '<path d="M6 6L18 18M6 18L18 6" stroke="#ef4444" stroke-width="2.5" stroke-linecap="round"/>';
  const icClip = '<rect x="5" y="4" width="14" height="17" rx="2" stroke="#a78bfa" stroke-width="2" fill="none"/><path d="M9 2H15V6H9V2Z" fill="#a78bfa"/>';

  const icClassWar = '<path d="M4 20L18 6M8 20L18 10" stroke="#fb7185" stroke-width="2.2" stroke-linecap="round"/><path d="M15 3L21 9L18 12L12 6Z" fill="#fb7185"/>';
  const icClassMag = '<path d="M12 2L14 9L21 11L14 13L12 20L10 13L3 11L10 9Z" fill="#60a5fa"/>';
  const icClassRog = '<path d="M4 20L16 8M16 8L14 4L20 6L16 8Z" fill="#f59e0b" stroke="#f59e0b" stroke-linejoin="round"/>';
  const icClassHea = '<path d="M12 21C12 21 4 14.5 4 9.5C4 6.5 6.5 4 9.5 4C11 4 12 5 12 5C12 5 13 4 14.5 4C17.5 4 20 6.5 20 9.5C20 14.5 12 21 12 21Z" fill="#34d399"/>';
  const icMoon = '<path d="M20 14.5A8.5 8.5 0 1 1 9.5 4a6.5 6.5 0 0 0 10.5 10.5Z" fill="#94a3b8"/>';
  const icCalendar = '<rect x="3" y="4" width="18" height="17" rx="2" stroke="#a5b4fc" stroke-width="1.8" fill="none"/><path d="M3 9.5H21" stroke="#a5b4fc" stroke-width="1.8"/><path d="M7 2.2V6M17 2.2V6" stroke="#a5b4fc" stroke-width="1.8" stroke-linecap="round"/>';

  const cfgMap = {
    warrior: { sec: '#fb7185', n: 'WARRIOR' },
    mage: { sec: '#60a5fa', n: 'ARCHMAGE' },
    rogue: { sec: '#fbbf24', n: 'SHADOW ROGUE' },
    healer: { sec: '#34d399', n: 'HIGH HEALER' },
  };
  ...
  const canvasW = 460;
  const QUOTE_Y = 1335;
  const quoteBoxH = 40 + Math.max(1, quoteLines.length) * 18 + 12;
  const canvasH = QUOTE_Y + quoteBoxH + 20;
  ...
  const h53dayStr = topH3day.map((it, i) => `<text x="28" y="${834 + i * 18}" class="list">${i + 1}. ${it.text} (+${it.count})</text>`).join('');
  const hnegStr = topHNeg3.map((it, i) => `<text x="28" y="${972 + i * 18}" class="list">${i + 1}. ${it.text} (-${it.count})</text>`).join('');
  const hStr = topH.map((it, i) => `<text x="28" y="${1072 + i * 18}" class="list">${i + 1}. ${trunc(it.text)} (+${it.count})</text>`).join('');
  const dStr = topD.map((it, i) => `<text x="248" y="${1072 + i * 18}" class="list">${i + 1}. ${trunc(it.text)} (${it.count}x)</text>`).join('');
  const hmonthStr = topHMonth.map((it, i) => `<text x="28" y="${1220 + i * 18}" class="list">${i + 1}. ${trunc(it.text)} (+${it.count})</text>`).join('');
  ...
  <defs>
    <clipPath id="rc"><rect width="${canvasW}" height="${canvasH}" rx="18"/></clipPath>
  ...
  <g clip-path="url(#rc)">
    ...
    <g transform="translate(128, 100) scale(0.75)" opacity="${db.classes_used.includes('warrior') ? '1.0' : '0.2'}">${icClassWar}</g>
    <g transform="translate(156, 100) scale(0.75)" opacity="${db.classes_used.includes('mage') ? '1.0' : '0.2'}">${icClassMag}</g>
    ...
    <rect x="236" y="226" width="208" height="46" rx="8" fill="url(#gC)" stroke="#4c1d2c"/><text x="246" y="242" class="l">PEAK DAILY RECORD</text><g transform="translate(246, 247) scale(0.8)">${icFr}</g><text x="270" y="261" class="v">${fmt(db.peak_daily_damage)}</text>
    <rect x="16" y="280" width="208" height="46" rx="8" fill="url(#gC)" stroke="#4c1d2c"/><text x="26" y="296" class="l">BOSSES SLAIN</text><g transform="translate(26, 301) scale(0.8)">${icTr}</g><text x="50" y="315" class="v">${db.bosses_slain}</text>
    <rect x="236" y="280" width="208" height="46" rx="8" fill="url(#gC)" stroke="#4c1d2c"/><text x="246" y="296" class="l">PEAK GOLD HOARDED</text><g transform="translate(246, 301) scale(0.8)">${icGd}</g><text x="270" y="315" class="v" fill="#fbbf24">${fmt(db.peak_gold)} G</text>
    ...
    <g transform="translate(26, 460) scale(0.7)">${icThumbUp}</g><text x="42" y="472" class="s">Habit Mastery: <tspan class="v">${hratio}% Positive</tspan></text>
    <g transform="translate(280, 460) scale(0.7)">${icThumbUp}</g><text x="296" y="472" class="s">${up}</text>
    <g transform="translate(330, 460) scale(0.7)">${icThumbDown}</g><text x="346" y="472" class="s">${dn}</text>

    <rect x="16" y="493" width="101" height="46" rx="7" fill="#1f1610" stroke="#b45309"/><text x="22" y="509" class="l">HABITS TODAY</text><g transform="translate(22, 513) scale(0.75)">${icSp}</g><text x="44" y="528" class="v">${hToday}</text>
    <rect x="125" y="493" width="101" height="46" rx="7" fill="#0d1f18" stroke="#059669"/><text x="131" y="509" class="l">DAILIES TODAY</text><g transform="translate(131, 513) scale(0.75)">${icCk}</g><text x="153" y="528" class="v">${done.length}</text>
    <rect x="234" y="493" width="101" height="46" rx="7" fill="#0f1f33" stroke="#0284c7"/><text x="240" y="509" class="l">TO-DOS TODAY</text><g transform="translate(240, 513) scale(0.75)">${icTg}</g><text x="262" y="528" class="v">${tToday}</text>
    <rect x="343" y="493" width="101" height="46" rx="7" fill="#241b0b" stroke="#ca8a04"/><text x="349" y="509" class="l">ALL COMPLETED</text><g transform="translate(349, 513) scale(0.75)">${icSt}</g><text x="371" y="528" class="v" fill="#fbbf24">${fmt(gTotal)}</text>

    <rect x="16" y="547" width="101" height="46" rx="7" fill="#0d2818" stroke="#16a34a"/><text x="22" y="563" class="l">HABITS POSITIF</text><g transform="translate(22, 567) scale(0.7)">${icThumbUp}</g><text x="40" y="581" class="v">${fmt(db.all_time_habits_pos)}</text>
    <rect x="125" y="547" width="101" height="46" rx="7" fill="#2a0f14" stroke="#dc2626"/><text x="131" y="563" class="l">HABITS NEGATIF</text><g transform="translate(131, 567) scale(0.7)">${icThumbDown}</g><text x="149" y="581" class="v">${fmt(db.all_time_habits_neg)}</text>

    <rect x="234" y="547" width="101" height="46" rx="7" fill="#2a1608" stroke="#ea580c"/><text x="240" y="563" class="l">DAILIES GAGAL</text><g transform="translate(240, 567) scale(0.75)">${icX}</g><text x="260" y="581" class="v">${dailiesGagal}</text>
    <rect x="343" y="547" width="101" height="46" rx="7" fill="#170f33" stroke="#7c3aed"/><text x="349" y="563" class="l">TODO BELUM</text><g transform="translate(349, 567) scale(0.7)">${icClip}</g><text x="367" y="581" class="v">${tActive}</text>
